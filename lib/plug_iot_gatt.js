...
exports.newInstance = async function($, spec) {
    try {
        ...
        that.__iot_reset__ = function() {
            ...
            Object.keys(allP).forEach((pId) => {
                ...
                if (p.state === 'connected') {
                    result.push(that.__iot_disconnect__(pId));
                }
            });
            ...
            const matchId = function(services) {
                ...
                services.some(function(x) {
                    ...
                    if ((typeof matchServiceId === 'string') &&
                        ...
                        matchServiceId = utilGatt.toNumber(serviceId);
                    }
                    if (utilGatt.toLowerCase(x.uuid) ===
                        utilGatt.toLowerCase(matchServiceId)) {
                        result = x;
                        return true;
                    } else {
                        return false;
        ...
        that.__iot_write__ = function(characteristic, value) {
            const bWrite = (cb) => {
                characteristic.write(value, true/*withoutResponse*/, cb);
            };
