...
exports.newInstance = function($, spec, cb) {
     ...
     * due to a timeout, or an object of type:
     *
     ...
     */
    that.findCharacteristics = function(serviceId, device, cb) {
        assert(typeof serviceId === 'string');
        assert(device && (typeof device === 'object'));
        assert(typeof cb === 'function');

        $._.__iot_findCharacteristics__(serviceId, device, cb);
    };
     ...
     * @param {number=} delay An optional delay in msec before disconnecting.
    ...
    that.disconnect = function(device, delay, cb) {
        assert(device && (typeof device === 'object'));
        $._.__iot_disconnect__(device, delay, cb);
    };
